...
export default function TagCard({
  ...
  const getStatusBadge = () => {
    switch (tag.status) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-950/80 text-rose-400 border border-rose-500/30">
  ...
  return (
    <div className="glass-card rounded-3xl p-5 border border-white/10 hover:border-slate-700 transition shadow-xl relative flex flex-col justify-between group">
      <div>
        {/* Top Bar: Tag ID & Status Switcher */}
        ...
        {/* Vehicle Details & QR Preview */}
        <div className="flex items-start justify-between gap-4 py-4">
          <div className="flex-1 min-w-0">
            {/* Number Plate Badge */}
            <div className="inline-block bg-yellow-400 text-black border-2 border-black font-black plate-font text-xl px-3 py-1 rounded-lg shadow-sm">
              {tag.vehicleNumber}
            </div>

            {/* Model & Type */}
            <div className="mt-2 text-sm font-semibold text-white truncate flex items-center gap-2">
              <Car className="w-4 h-4 text-brand-400 shrink-0" />
              <span>{tag.vehicleModel || 'Vehicle'}</span>
            </div>

            {/* Linked Phone with Masking & Dynamic Edit */}
            <div className="mt-3 p-2.5 rounded-xl bg-slate-900/90 border border-slate-800">
              <div className="text-[10px] uppercase font-bold text-slate-400 flex items-center justify-between mb-1">
                ...
                {!isEditingPhone && (
                  <button
                    ...
                    title={showFullPhone ? 'Mask Number' : 'Show Full Number'}
                  >
              ...
              {isEditingPhone ? (
                <div className="flex items-center gap-1.5 mt-1">
                  <input
                    ...
                    className="w-full bg-slate-950 border border-brand-500 rounded-lg px-2 py-1 text-xs text-white focus:outline-none"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[10px] text-brand-400 hover:underline font-semibold flex items-center gap-0.5"
                  >
          ...
          <div className="shrink-0 flex flex-col items-center bg-white p-2 rounded-xl shadow-md border border-slate-200">
            ...
            <span className="text-[8px] font-bold text-slate-800 uppercase tracking-tight mt-1">
              Live QR
            </span>
          </div>
        </div>
        ...
        {/* Parking Status Note */}
        <div className="text-xs text-slate-400 bg-slate-900/40 p-2 rounded-xl border border-white/5 flex items-start justify-between gap-2">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400 mt-3 pt-3 border-t border-white/5">
          ...
          <div className="flex items-center gap-1 text-slate-500">
            ...
            <span>{tag.lastScannedAt ? 'Recent' : 'Never'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-white/10">
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-brand-500/10 hover:bg-brand-500/20 text-brand-400 border border-brand-500/30 text-xs font-bold transition"
        >
